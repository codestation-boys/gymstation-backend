...
4 - after you finish installing the dependencies, you can boot with `docker-compose up` (Should appear `server is running...` in your terminal).  
5 - if everything went well, make a `POST` request for `http://localhost:{port}/accounts` with yout user informations.  
